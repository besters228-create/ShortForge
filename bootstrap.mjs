...
async function restore(name,target){
  ...
  const parts=(await fs.readdir(dir)).filter(x=>x.startsWith(name+".part")).sort();
  if(!parts.length) throw new Error("Missing payload for "+name);
  let b64=""; for(const p of parts) b64+=await fs.readFile(path.join(dir,p),"utf8");
  const data=gunzipSync(Buffer.from(b64,"base64"));
  const out=path.join(root,target); await fs.mkdir(path.dirname(out),{recursive:true}); await fs.writeFile(out,data);
}
