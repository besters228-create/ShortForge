...
async function restore(name,target){
  const b64=await fs.readFile(path.join(root,"payload",name+".br.b64"),"utf8");
...
try{ await fs.access(path.join(root,"catalog","topics_1000.json")); }
catch{ await fs.writeFile(path.join(root,"catalog","topics_1000.json"),JSON.stringify({version:"9.7.11",count:0,topics:[]})); }
